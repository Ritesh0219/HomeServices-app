...
function CategorySideBar() {
  ...
  return (
    <div className="flex flex-col gap-1 h-[80vh]  justify-center">
      <h2 className='text-primary mb-2 font-bold text-center w-[30%] px-2 bg-secondary p-2 rounded-full'>
        Categories
      </h2>
      {categoryList.length === 0 ? (
        ...
        categoryList.map((category) => (
          <Link href={`/search/${encodeURIComponent(category.name)}`} key={category.id}>
            <div 
              className={`flex items-center p-4 md:py-4 lg:py-4 gap-3 border hover:bg-secondary hover:text-primary hover:border-primary cursor-pointer rounded-md ${selectedCategory==category.name&&'border-primary text-primary bg-secondary'}`} 
            >
              <Image
      ...
      <Link href='/contact' className='w-[40%] mt-1'>
      <h2 className='text-white px-2 mb-2 font-bold text-center  hover:bg-orange-800 bg-primary p-2 rounded-full'>
        Contact Us
      </h2>
      </Link>
